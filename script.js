document.addEventListener("DOMContentLoaded", function () {
  ...
  var rules = {
    ...
    consent: { required: true, label: "La case RGPD" }
  };
  ...
  function validateField(field) {
    ...
    var value = isCheckbox ? (field.checked ? "checked" : "") : field.value.trim();
    var errorText = "";
 
    if (rule.required && value.length === 0) {
      errorText = isCheckbox ? "Merci d'accepter les conditions pour continuer." : rule.label + " est obligatoire.";
  ...
  Object.keys(rules).forEach(function (name) {
    ...
    if (!field) return;
    var evtName = field.type === "checkbox" ? "change" : "blur";
    field.addEventListener(evtName, function () { validateField(field); });
  });
